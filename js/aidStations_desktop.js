function filterAidStationTents(obj){
	return ((obj.Type=="AS")&&obj.Location!="Indiana");
};
...
function drawAidStationTents(data){
	...
	var MedicalTentsDataset = data.filter(filterAidStationTents);
	...
	var w = document.getElementById('aidStations').offsetWidth * 0.93;
	var h = document.getElementById('aidStations').offsetHeight*0.75;
	//padding
	var p = 10;
	var xPad = 10;
	...
	var svg = d3.select(".aidStations")
				.append("svg")
				.attr("width",w+2*p)
				.attr("height",h+3*p)
				.attr("class","aid_station_mt1")
				.attr("id","medical_tent1");
	...
	svg.append("g")
		.attr("class","x axis")
		.attr("transform","translate(" + xPad + ","+(h-p)+")") //moves it to the right coordinate
	.call(xAxis);

		
			var barsTotal = svg.selectAll(".totalbeds1")
				.data(MedicalTentsDataset)
				.enter()
				.append("rect")
				.attr("class", "totalbeds1")
				.attr("x",function(d,i){
					return p+xPad;
				})
				.attr("y",function(d,i){
...
d3.csv('data/AidStations.csv',drawAidStationTents);
